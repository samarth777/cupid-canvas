...
function Home() {
  ...
  async function handleClick() {
    ...
    setIsLoading(true);
    try { 
      const response = await axios.post("https://cupid-canvas.onrender.com/generate", {
      ...
      console.log(response);
      navigate("/result?src=" + encodeURIComponent("https://cupid-canvas.onrender.com/"+response.data.image_paths));
    } catch (error) {
      console.error("Error fetching data: ", error);
    } finally {
      setIsLoading(false);
